...
TrainingRoomPanel::TrainingRoomPanel(const std::vector<ModelInfo> &models,
                                     QWidget *parent)
    : QWidget(parent), availableModels(models) {
  setupUI();
  waveTimer = new QTimer(this);
  connect(waveTimer, &QTimer::timeout, this,
          &TrainingRoomPanel::updateWaveform);
  waveTimer->start(30);
}
...
QWidget *TrainingRoomPanel::createHeader() {
  ...
  deployBtn->setStyleSheet(
      "background-color: #cfbcff; color: #381e72; border-radius: 4px; "
...
QFrame *
TrainingRoomPanel::createModelSelector(const std::vector<ModelInfo> &models) {
  QFrame *f = new QFrame();
  ...
  title->setStyleSheet("font-weight: bold; font-size: 12px; color: #ffffff; "
  ...
  if (models.empty()) {
    ...
    for (size_t i = 0; i < models.size(); ++i) {
      ...
      l->addSpacing(5);
      if (i >= 2)
        break; // Limit to 3 for UI spacing
    }
  }
  ...
  readiness->setStyleSheet(
      "font-size: 9px; color: #938f99; font-weight: bold; border: none;");
  ...
  pb->setStyleSheet(
      "QProgressBar { background-color: #25232a; border: none; border-radius: "
      "2px; } QProgressBar::chunk { background-color: #cfbcff; }");
  l->addWidget(pb);
...
QFrame *TrainingRoomPanel::createEmotionLibrary() {
  ...
  auto createCard = [](const QString &name, const QString &status,
                       const QString &color) {
    QFrame *c = new QFrame();
    c->setFixedHeight(120);
    c->setStyleSheet(QString("background-color: #141218; border: 1px solid %1; "
                             "border-radius: 10px;")
                         .arg(color));
    ...
    tag->setStyleSheet(
        QString("color: %1; font-size: 8px; font-weight: bold; text-transform: "
    ...
    n->setStyleSheet("font-size: 15px; font-weight: bold; color: #ffffff; "
                     "border:none; background:transparent;");
    cl->addWidget(n);

    QLabel *d =
        new QLabel("Recursive emotional resonance and shared perspective.");
    d->setStyleSheet(
        "font-size: 9px; color: #938f99; border:none; background:transparent;");
    d->setWordWrap(true);
    cl->addWidget(d);
...
QFrame *TrainingRoomPanel::createPrerequisites() {
  ...
  title->setStyleSheet("font-weight: bold; font-size: 9px; color: #938f99; "
  ...
  auto addRow = [&](const QString &text, const QString &status, bool locked) {
    ...
    s->setStyleSheet("font-size: 8px; color: #938f99; text-transform: "
...
QFrame *TrainingRoomPanel::createWaveformPreview() {
  ...
  title->setStyleSheet(
      "font-weight: bold; font-size: 9px; color: #938f99; border: none;");
  l->addWidget(title);
...
QFrame *TrainingRoomPanel::createSummaryCard() {
  ...
  QLabel *msg =
      new QLabel("Training empathy will utilize 4.2GB of neural memory and is "
                 "expected to take 14 cycles of reinforcement.");
...
QFrame *TrainingRoomPanel::createTerminal() {
  ...
  terminalOutput->append(QString("<span style='color:#938f99'>[%1]</span> "
                                 "<span style='color:#cfbcff'>[SYSTEM]</span> "
                                 "SENTI-9 kernel verified. Handshake success.")
                             .arg(ts));
  terminalOutput->append(
      QString("<span style='color:#938f99'>[%1]</span> <span "
              "style='color:#2dd4bf'>[NEURAL]</span> Pre-requisite check for "
              "'Empathy' initiated...")
          .arg(ts));
  terminalOutput->append(
      QString("<span style='color:#938f99'>[%1]</span> <span "
              "style='color:#2dd4bf'>[NEURAL]</span> Recognition module: <span "
              "style='color:#ffffff'>OPTIMAL (0.992 fidelity)</span>")
          .arg(ts));
  terminalOutput->append(QString("<span style='color:#938f99'>[%1]</span> "
                                 "<span style='color:#e7c365'>[WAIT]</span> "
                                 "Awaiting user initialization signal...")
                             .arg(ts));
